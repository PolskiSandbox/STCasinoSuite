...
function ENT:Use(activator, caller)
	if !table.HasValue(self.Entrants, caller) then
		if caller:getDarkRPVar( "money" ) >= (STC_CONFIG.GasCashCost) then
			...
			caller:CasinoPayout( -STC_CONFIG.GasCashCost )
			self:SetNWInt("winnings", STC_CONFIG.GasCashCost * 2)
...
function ENT:Think()
	self:SetNWInt("winnings", math.Clamp(self:GetNWInt("winnings") - STC_CONFIG.GasCashCost/50, 0, STC_CONFIG.GasCashCost * 2))
