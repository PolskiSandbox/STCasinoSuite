...
function ENT:Use(activator, caller)
	...
	if caller:getDarkRPVar( "money" ) >= STC_CONFIG.JPCost then
		local payout = 0
		...
		if self.active == false then
			...
			caller:CasinoPayout( -STC_CONFIG.JPCost )
			file.Write("stcasino/jacks_jackpot.txt", tostring(tonumber(self:GetNWInt("jackpot")) + STC_CONFIG.JPAdd))
			...
			timer.Simple(3.1, function()
				local payouts = {0, STC_CONFIG.JPRibbon, STC_CONFIG.JPCoins, STC_CONFIG.JPCashEach, STC_CONFIG.JPLightning, STC_CONFIG.JPMusic, STC_CONFIG.JP8Ball, 3000, STC_CONFIG.JPStar}
				--TRIPLE ITEM--
				...
				if s1 != 1 and s2 != 1 and s3 != 1 then
					if s1 == 4 then
						payout = payout + STC_CONFIG.JPCashEach
